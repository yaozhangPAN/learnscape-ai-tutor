  ...
  return (
    <div className="min-h-screen bg-[#FFF7D1] flex flex-col">
      ...
      <div className="flex-1 w-full max-w-5xl px-2 mx-auto py-6">
            ...
            {leaderboardData.slice(0, 3).map((user) => (
              ...
              >
                ...
                <div className="flex flex-col items-center bg-white/70 rounded-2xl mx-5 py-4 shadow-inner">
                  ...
                  <span className="text-xs text-[#91712A]">Points</span>
                  <span className="mt-1 text-[#916A13]">{user.questions} Questions Answered</span>
                </div>
                {user.rank === 1 && (
                  <span className="absolute -top-4 left-1/2 -translate-x-1/2 bg-[#FFDA7F] rounded-full px-4 py-1 font-bold text-[#AF821A] shadow-lg">🏆 Champion</span>
                )}
                {user.rank === 2 && (
                  <span className="absolute -top-4 left-1/2 -translate-x-1/2 bg-[#EFEFEF] rounded-full px-4 py-1 font-bold text-[#909497] shadow-lg">🥈 Runner-up</span>
                )}
                {user.rank === 3 && (
                  <span className="absolute -top-4 left-1/2 -translate-x-1/2 bg-[#FFD5A6] rounded-full px-4 py-1 font-bold text-[#C87D3F] shadow-lg">🥉 3rd Place</span>
                )}
              </div>
            ))}
          </div>
        ...
        {/* Leaderboard table（高亮行/条纹，色块背景，圆角，可爱边框） */}
        <div className="bg-white/90 rounded-3xl shadow-xl border-2 border-[#FFE29F] p-0 overflow-hidden animate-fade-in">
          <Table>
            <TableHeader>
              <TableRow>
                <TableHead className="w-16 text-[#A37A36]">Rank</TableHead>
                <TableHead className="text-[#A37A36]">User</TableHead>
                <TableHead className="text-[#A37A36]">Points</TableHead>
                <TableHead className="hidden md:table-cell text-[#A37A36]">Questions</TableHead>
                <TableHead className="hidden md:table-cell text-[#A37A36]">Badge</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
